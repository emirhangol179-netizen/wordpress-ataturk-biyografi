...
class OsStripeConnectHelper {
	...
	public static function output_stripe_link_on_customer_quick_form( OsCustomerModel $customer ) {
		...
		if ( $stripe_customer_id ) {
			echo '<div class="payment-processor-customer-link-wrapper">' . esc_html__( 'Stripe Customer', 'latepoint' ) . '<a target="_blank" href="' . esc_url( self::build_customer_profile_link( $stripe_customer_id, OsSettingsHelper::is_env_payments_dev() ) ) . '">' . esc_html__( 'Open in Stripe', 'latepoint' ) . '</a></div>';
		}
	}
	...
	public static function retrieve_payment_intent( string $payment_intent_id ): array {
		...
		$result               = [
			'id'     => $payment_request_data['data']['id'],
			'status' => $payment_request_data['data']['status'],
	...
	public static function generate_payment_intent_id_and_secret_for_transaction_intent( OsTransactionIntentModel $transaction_intent ): array {
		...
		if ( empty( $result['data'] ) ) {
			...
			if ( $result['data']['stripe_customer_id'] != self::get_stripe_customer_id( $order->customer ) ) {
				self::save_stripe_customer_id( $order->customer, $result['data']['stripe_customer_id'] );
			}
	...
	public static function generate_payment_intent_id_and_secret_for_order_intent( OsOrderIntentModel $order_intent ): array {
		$customer_data = [
			'name'  => $order_intent->customer->full_name,
			'email' => $order_intent->customer->email,
		];
		$options       = [
		...
		if ( empty( $result['data'] ) ) {
			...
			// make sure we use correct stripe customer id in case the one that was passed is invalid - a valid one will be returned in this call
			if ( $result['data']['stripe_customer_id'] != self::get_stripe_customer_id( $order_intent->customer ) ) {
				self::save_stripe_customer_id( $order_intent->customer, $result['data']['stripe_customer_id'] );
			}
