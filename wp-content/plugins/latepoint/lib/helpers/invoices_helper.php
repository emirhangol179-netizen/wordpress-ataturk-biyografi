...
class OsInvoicesHelper {
	...
	public static function handle_invoice_updated( OsInvoiceModel $new_invoice, OsInvoiceModel $old_invoice ) {
		...
		$objects[] = [
			'model'       => 'invoice',
			'id'          => $new_invoice->id,
			'model_ready' => $new_invoice,
		];
		$objects[] = [
			'model'       => 'old_invoice',
			'id'          => $old_invoice->id,
			'model_ready' => $old_invoice,
		];
	...
	public static function log_invoice_updated( OsInvoiceModel $invoice, OsInvoiceModel $old_invoice ) {
		...
		$data['description'] = wp_json_encode(
			[
				'invoice_data_vars' => [
	...
	public static function generate_invoice_data_from_order( OsOrderModel $order ): array {
		$data = [
			...
			'from'            => OsReplacerHelper::replace_all_vars(
				OsInvoicesHelper::get_invoice_data_bill_from(),
				[
					'order'    => $order,
	...
	public static function add_conditions_for_invoice_events( array $objects, string $event_type ): array {
		switch ( $event_type ) {
			case 'invoice_created':
				$objects[] = [
					'code'       => 'invoice',
					'model'      => 'OsInvoiceModel',
					'label'      => __( 'Invoice', 'latepoint' ),
					'properties' => [],
				];
				break;
			case 'invoice_updated':
				$objects[] = [
					'code'       => 'old_invoice',
					'model'      => 'OsInvoiceModel',
					'label'      => __( 'Old Invoice', 'latepoint' ),
					'properties' => [],
				];
				$objects[] = [
					'code'       => 'invoice',
					'model'      => 'OsInvoiceModel',
					'label'      => __( 'New Invoice', 'latepoint' ),
					'properties' => [],
				];
				break;
		}
	...
	public static function add_settings_sticky_menu_item( array $items ): array {
		$items[] = [
			'href'  => 'stickySectionInvoice',
			'label' => __( 'Invoices', 'latepoint' ),
		];
		return $items;
	}
