		...
		*/
		...
		public function widget_daily_bookings_chart( $date_from = false, $date_to = false ) {
			...
			if ( ! empty( $this->params['agent_id'] ) ) {
				$filter->agent_id = $this->params['agent_id'];
			}
			if ( ! empty( $this->params['service_id'] ) ) {
				$filter->service_id = $this->params['service_id'];
			}
			if ( ! empty( $this->params['location_id'] ) ) {
				$filter->location_id = $this->params['location_id'];
			}

			if ( ! OsRolesHelper::are_all_records_allowed() ) {
				if ( ! OsRolesHelper::are_all_records_allowed( 'agent' ) ) {
					$agent_id = OsRolesHelper::get_allowed_records( 'agent' );
				}
			}
		...
		public function widget_bookings_and_availability_timeline() {
			...
			if ( $agents_models ) {
				foreach ( $agents_models as $agent ) {
					...
					if ( $selected_location_id ) {
						$args['location_id'] = $selected_location_id;
					}
				...
				foreach ( $availability_resources[ $target_date->format( 'Y-m-d' ) ] as $resource ) {
					if ( isset( $agent_work_time_periods[ $resource->agent_id ] ) ) {
